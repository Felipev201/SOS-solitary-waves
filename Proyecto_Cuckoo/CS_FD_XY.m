function [bestnest]=CS_FD_XY(a,lambda,DA)
if nargin<1
   lambda=1;a=10;DA=1; 
elseif nargin<2
   lambda=1;DA=1;
...
Lb=0.5.*ones(1,nd);%Lower bounds
Ub=5.*ones(1,nd);%.*ones(1,nd);%Upper bounds
...
[fmin,bestnest,nest,fitness]=get_best_nest(nest,nest,fitness,du,kx,ky,X,Y,theta,lambda,dx,N,DA);
N_iter=0;
iter=1;
%% Starting iterations
while (iter<N_IterTotal)% && fmin>9e-3) 
    % Generate new solutions (but keep the current best)
     new_nest=get_cuckoos(nest,bestnest,Lb,Ub);   
     [fnew,best,nest,fitness]=get_best_nest(nest,new_nest,fitness,du,kx,ky,X,Y,theta,lambda,dx,N,DA);
    % Update the counter
      N_iter=N_iter+n; 
...
function [fmin,best,nest,fitness]=get_best_nest(nest,newnest,fitness,du,kx,ky,X,Y,theta,lambda,dx,N,DA)
% Evaluating all new solutions
    newnest4=permute(newnest,[4,2,3,1]);
    fnew=fobj(newnest4,du,kx,ky,X,Y,theta,lambda,dx,N,DA);
...
%% Fitness Funtion
function z=fobj(u,du,kx,ky,X,Y,theta,lambda,dx,n,DA)
...
%% Proposed Ansatz
m=0;
U_0=u(1,1,:,:).*exp(-((X-6).^2+Y.^2))./u(1,2,:,:).^2.*exp(1i.*m.*theta).*((X-6).^2+Y.^2).^m;
m=1;
U_1=u(1,3,:,:).*exp(-((X+6).^2+Y.^2))./u(1,4,:,:).^2.*exp(1i.*m.*theta).*((X+6).^2+Y.^2).^m;
m=2;
U_2=u(1,5,:,:).*exp(-(X.^2+(Y+6).^2))./u(1,6,:,:).^2.*exp(1i.*m.*theta).*(X.^2+(Y+6).^2).^m;
U=U_0+U_1+U_2;
