function [bestnest]=CS_FD(a,lambda,DA)
if nargin<1
   lambda=1;a=15;DA=1; 
elseif nargin<2
   lambda=1;DA=1;
...
Lb=[0.5,0.5];
Ub=[20,10];%.*ones(1,nd);%Upper bounds
%Ub=ones(1,6).*20;
...
%% Proposed Ansatz
m=1;
%U=u(1,1,:,:).*exp(-rho2./u(1,2,:,:).^2).*exp(i.*m.*theta).*rho.^m;
 U=u(1,1,:,:).*sech(rho./u(1,2,:,:)).*exp(i.*m.*theta).*rho.^m;
%  U1=u(1,1,:,:).*exp(-rho2./u(1,2,:,:).^2).*rho2.*(cos(2.*theta)+1i*0.*sin(2*theta));
%  U2=u(1,3,:,:).*exp(-rho2./u(1,4,:,:).^2).*rho2.*(cos(2.*theta)+1i*.5.*sin(2*theta));
%  U3=u(1,5,:,:).*exp(-rho2./u(1,6,:,:).^2).*rho2.*(cos(2.*theta)+1i*.75.*sin(2*theta));
%U=u(1,1,:,:).*exp(-rho2./u(1,2,:,:).^2).*rho2.*(cosd(40).*cos(2.*theta)+1i.*sind(40).*sin(2.*theta)); 
% U=U1+U2+U3;
...
%% Medium
NL=-1/2.*U2.^2;                      %Kerr
%s=0.05;NL=(log(s.*U2+1)-s.*U2)./s^2; %Saturation
%s=0.2
%s=0.2;NL_1=(log(s.*U2+1)-s.*U2)./s^2;...
%p=15;k=3;NL_2=p.*(besselj(5,k.*rho)).^2.*U2;
%s=0.2;NL_1=(log(s.*U2+1)-s.*U2)./s^2;...
%p=15;k=3;NL_2=p.*(besselj(5,k.*rho)).^2.*U2;
%NL=NL_1+NL_2;
...
%U1=A*exp(-r.^2/B.^2).*r.^2.*(cos(2*th)+1i*0*sin(2*th));
%U2=A*exp(-r.^2/B.^2).*r.^2.*(cos(2*th)+1i*.5*sin(2*th));
%U3=A*exp(-r.^2/B.^2).*r.^2.*(cos(2*th)+1i*.75*sin(2*th));
%U=U1+U2+U3;
%s=0.2
